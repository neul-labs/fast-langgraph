...
def enable_all_optimizations(verbose: bool = True) -> Dict[str, bool]:
    ...
    try:
        ...
        if verbose:
            print("⚠ Warning: RustCheckpointer not available (Rust extension not built)")
...
def print_optimization_report():
    ...
    print(f"  Thread Pool Cache:     {'✓' if status['executor_cache_enabled'] else '✗'}")
    print(f"  Accelerated Algorithms: {'✓' if status['shim_enabled'] else '✗'}")
    print(f"  RustCheckpointer:      {'✓' if status['rust_checkpoint_available'] else '✗'}")
    print()
