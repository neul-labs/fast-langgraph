...
impl<T: Clone + Send + Sync + 'static, U: Clone + Send + Sync + 'static> PregelExecutor<T, U> {
    ...
    pub async fn execute_until_completion(&self) -> Result<Vec<PregelTaskWrites<U>>, LangGraphError> {
        let start_time = std::time::Instant::now();
    ...
    async fn execute_tasks(
        ...
        for future in task_futures {
            match future.await {
                ...
                Err(e) => return Err(LangGraphError::NodeExecutionError {
...
impl<T: Clone + Send + Sync + 'static, U: Clone + Send + Sync + 'static> Default for PregelExecutor<T, U> {
    fn default() -> Self {
        Self::new()
    }
}
