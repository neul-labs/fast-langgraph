...
impl<T: Clone + Send + Sync + Serialize + for<'de> Deserialize<'de>> Channel<T, T> for LastValueChannel<T> {
    fn get(&self) -> Result<&T, LangGraphError> {
        self.value.as_ref().ok_or(LangGraphError::ChannelError("Channel is empty".to_string()))
    }
    ...
    fn memory_usage(&self) -> usize {
        ...
        self.value.as_ref().map(|v| std::mem::size_of_val(v)).unwrap_or(0)
    }
}
...
impl<T: Clone + Send + Sync + Serialize + for<'de> Deserialize<'de>> Channel<Vec<T>, T> for TopicChannel<T> {
    ...
    fn memory_usage(&self) -> usize {
        ...
        self.values.iter().map(|v| std::mem::size_of_val(v)).sum::<usize>()
    }
}
